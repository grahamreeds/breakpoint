...
This is a modification of [Scottt Debugbreak](https://github.com/scottt/debugbreak) which changes the name to match that of the 2026 addition to the C++ standard[*](https://en.cppreference.com/w/cpp/utility/breakpoint.html).

[debugging.h](https://github.com/grahamreeds/breakpoint/blob/master/debugging.h) allows you to put breakpoints in your C/C++ code with a call to **breakpoint()**:
```C
#include <stdio.h>
...
int main()
{
	breakpoint(); /* will break into debugger */
	printf("hello world\n");
	return 0;
}
```
I have added the functions `is_debugger_present()` and `breakpoint_if_debugging()` though these are less complete than Scott's.

* Include one header file and insert calls to `breakpoint()` in the code where you wish to break into the debugger.
...
The requirements for the **breakpoint()** function are:
...
* GDB commands like **continue**, **next**, **step**, **stepi** must work after a **breakpoint()** hit
...
**breakpoint()** generates an **int3** instruction on i386 / x86-64 ([test/break.c](https://github.com/scottt/debugbreak/blob/master/test/break.c)):
```C
#include <stdio.h>
#include "debugbreak.h"
...
```
which correctly trigges **SIGTRAP** and single-stepping in GDB after a **breakpoint()** hit works well.
...
On ARM, **breakpoint()** generates **.inst 0xe7f001f0** in ARM mode and **.inst 0xde01** in Thumb mode which correctly triggers **SIGTRAP** on Linux. Unfortunately, stepping in GDB after a **breakpoint()** hit doesn't work and requires a workaround like:
```
(gdb) set $l = 2
(gdb) tbreak *($pc + $l)
(gdb) jump   *($pc + $l)
...
On AArch64, **breakpoint()** generates **.inst 0xd4200000**.

See table below for the behavior of **breakpoint()** on other architecturs.
